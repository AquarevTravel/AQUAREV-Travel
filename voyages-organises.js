...
const value=el.dataset[`placeholder${safeLang.toUpperCase()}`];
...
addDepartureDateBtn.addEventListener("click",()=>createDepartureDateRow());
...
return{departureDate:departure,returnDate:returnValue};
...
try{
let uploadLabel="";
if(type==="images")uploadLabel=`Téléversement de l'image ${index+1}/${total}...`;
else if(type==="videos")uploadLabel=`Téléversement de la vidéo ${index+1}/${total}...`;
else if(type==="pdf")uploadLabel="Téléversement du PDF...";
...
if(!result||!result.url)throw new Error("ImageKit n'a pas retourné l'URL du fichier.");
...
const uploadedFile=await uploadFileToImageKit(files[index],programId,type,index,files.length);
...
const allDepartureDates=[departure,...additionalPairs.map(item=>item.departureDate)];
...
if(!document.getElementById("hotel").value.trim()){
showStatus("Veuillez saisir le nom de l'hôtel.","error");
return false;
}

if(!document.getElementById("hotelStars").value){
showStatus("Veuillez sélectionner le nombre d'étoiles.","error");
return false;
}

if(!document.getElementById("hotelAddress").value.trim()){
showStatus("Veuillez saisir l'adresse de l'hôtel.","error");
return false;
}
...
const additionalPairs=getAdditionalDatePairs();
const departureDates=[departureDate.value,...additionalPairs.map(item=>item.departureDate)];
const returnDates=[returnDate.value,...additionalPairs.map(item=>item.returnDate)];
...
const programRef=await addDoc(collection(db,COLLECTION_NAME),programData);
...
images=await uploadFiles(imageFiles,programRef.id,"images");
}

if(videoFiles.length){
videos=await uploadFiles(videoFiles,programRef.id,"videos");
}

if(pdfFiles.length){
const uploadedPdf=await uploadFiles(pdfFiles,programRef.id,"pdf");
pdf=uploadedPdf[0]||null;
}

showStatus("Finalisation du programme...");

await updateDoc(doc(db,COLLECTION_NAME,programRef.id),{
images:images,
videos:videos,
pdf:pdf,
...
console.log("PROGRAM PUBLISHED:",programRef.id);
